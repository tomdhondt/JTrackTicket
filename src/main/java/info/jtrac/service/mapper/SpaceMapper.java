...
public class SpaceMapper extends AbstractMapper<Space, SpaceDTO>{
	...
	public Space mapToObject(SpaceDTO dto) throws MapperException {
		...
		IManager<Metadata> iMetadataManager = (IManager<Metadata>) context.getBean("iMetadataManager");
		IManager<SpaceSequence> iSpaceSequenceManager = (IManager<SpaceSequence>) context.getBean("iSpaceSequenceManager");
		Space object = null;
		if(null != dto){
			object = new Space();
			...
			/* Map the Metadata */
			try{
				Metadata mObject = iMetadataManager.findByID(MappingUtil.parseInt(dto.getMd_Id()));
			...
			try {
				SpaceSequence sObject = iSpaceSequenceManager.findByID(MappingUtil.parseInt(dto.getSpaceSequence_id()));
