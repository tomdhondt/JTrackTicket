...
public class ItemDAOImplTest {
	...
	SessionFactory session  = (SessionFactory) context.getBean("sessionFactoryJTrack");
	private Space space;
...
//		Query queryOpen = session.getCurrentSession().getNamedQuery("Item.count.open");
//		Query queryClosed = session.getCurrentSession().getNamedQuery("Item.count.closed");
//		Query queryNew = session.getCurrentSession().getNamedQuery("Item.count.new");
//		List<Long> objectOpen = queryOpen.list();
//		List<Long> objectClosed = queryClosed.list();
//		List<Long> objectNew = queryNew.list();
//		System.out.println("Open : " + objectOpen.get(0));
//		System.out.println("Closed : " + objectClosed.get(0));
//		System.out.println("New : " + objectNew.get(0));
//		session.getCurrentSession().getTransaction().commit();
//	}
	@Test
	public void findByCriteria(){
		try {
			List<Object> resultOpen = iPersistenceItemDAOImpl.findByCriteria("Item.count.open");
			List<Object> resultClosed = iPersistenceItemDAOImpl.findByCriteria("Item.count.closed");
			List<Object> resultNew = iPersistenceItemDAOImpl.findByCriteria("Item.count.new");
			Long countOpen = (Long)resultOpen.get(0);
			Long countClosed = (Long)resultClosed.get(0);
			Long countNew = (Long)resultNew.get(0);
